...
Usage
-----
    # Local only (default): no Qwoted HTTP. Writes ~/.qwoted/drafts/.
    python3 qwoted_pitch.py \\
        --source-request-id 235897 \\
        --pitch-text "Hi! I'm …"

    # Create a server-side draft (API traffic, reporter is NOT notified):
    python3 qwoted_pitch.py \\
        --source-request-id 235897 \\
...
def _fetch_opportunity_page(
    ...
    r = requests.get(
        url,
        cookies=cookies,
        headers={"User-Agent": effective_user_agent(), "Accept": "text/html,*/*"},
        timeout=30,
...
def send_pitch(
    *,
    ...
    # Default: write a local draft only. Creating a Qwoted draft still
    # POSTs /api/internal/pitches — that is not free of automation signal.
    if not send and not create_draft:
        ident = source_request_id or opportunity_id or "unknown"
        out = drafts_dir() / f"{ident}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
        payload = {
            "status": "local_draft",
            ...
            "attachment_paths": [a["path"] for a in prepared_attachments],
            "written_at": iso_now(),
            "note": "Not sent to Qwoted. Pass --create-draft to open a server draft, or --send to notify the journalist.",
        }
        out.write_text(json.dumps(payload, indent=2, default=str))
        log(f"LOCAL DRAFT only — wrote {out}. No Qwoted HTTP.")
        return {
            "status": "local_draft",
            "source_request_id": source_request_id,
            "page_url": None,
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--create-draft", action="store_true",
                   help="Create/autosave a draft on Qwoted (API traffic). "
                        "Default is a local file under ~/.qwoted/drafts/ with zero HTTP.")
    p.add_argument("--send", action="store_true",
                   help="Actually submit the pitch (notifies the reporter). "
                        "Implies API traffic. Never pass this unless the user named the ID.")
