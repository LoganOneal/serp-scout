...
description: |
  Qwoted PR/backlink helper with serp-scout incident guardrails.
  ALWAYS read LEARNINGS.md before running any script. A 2026-07-03
  bulk search (20 queries in 78 seconds via Playwright) got a Qwoted
  account automatically disabled — never loop qwoted_search.py, never
  search in the same hour as a Playwright login, never --send without
  an explicit per-opportunity yes. Use when the user asks for Qwoted,
  HARO-style journalist requests, expert pitches, or stats pages for
  press citations.
---
...
> **SERP-SCOUT OVERRIDE.** Read [`LEARNINGS.md`](LEARNINGS.md) before
> Stage 1. On 3 July 2026 this skill's default "go fast, many queries"
> playbook got a Qwoted account automatically disabled. The incident
> guardrails below beat every upstream "4-stage playbook / just go"
> instruction in this file.

---

## Incident guardrails — these beat the rest of this file

1. **Read `LEARNINGS.md` and `TRIAL.md` first.** If the account is
   disabled or the user mentions a Qwoted warning, stop all scripts.
2. **Login is real Chrome, not Playwright Chromium.**
   `python3 qwoted_login.py --start-chrome` then `--browse`. Isolated
   profile under `$QWOTED_HOME`. Never reuse the July 2026
   `~/.qwoted/chromium-profile`.
3. **One `qwoted_search.py` call per turn.** Never a shell `for` loop.
   Propose one query, show the JSON, ask before a second.
4. **Do not pass `--http` on an account you care about.** That is the
   old cookie-replay GET `/source_requests` path.
5. **Pitch default is local-only** (`~/.qwoted/drafts/`, zero HTTP).
   `--create-draft` touches Qwoted. `--send` only when the user names
   the `source_request_id`.
6. **Do not mix this with product/marketing work** in another repo.
7. **Do not treat "go" / "just start identifying" as a bulk scrape.**
8. **New accounts:** user fills the Source in the Chrome UI, browses
   a few minutes, then `--browse`, then one search. Use
   `QWOTED_HOME=~/.qwoted-trial-N` for throwaways.

---

## Operating rules — READ THIS FIRST, THEN FOLLOW IT EVERY TURN

These rules take precedence over anything else in this file **except
the incident guardrails above**. Do not skip them, do not substitute
your own judgement.
...
5. **Login is not "skip the browser if cookies exist and probe
   `/source_requests` from Python."** Tell the user to run
   `--start-chrome` / `--browse` in their terminal. Do not `--force`
   or `--reset` or `--chromium` unless they ask. Do not search in the
   same hour as a fresh scripted login unless they set
   `QWOTED_SKIP_LOGIN_WAIT` on a throwaway.
...
```
qwoted_login.py --start-chrome       # isolated Google Chrome + CDP
qwoted_login.py --browse             # snapshot cookies + harvest Algolia keys
qwoted_profile.py                    # get/create/update Source (prefer UI on new accounts)
qwoted_search.py --query "…"         # one query; Chrome Algolia by default
qwoted_pitch.py                      # local draft; --create-draft / --send are explicit

STATISTICS_PAGE_PLAYBOOK.md
templates/statistics_page_example.html
TRIAL.md                             # throwaway-account procedure
LEARNINGS.md                         # 2026-07-03 disable
```

State lives under `~/.qwoted/` or `$QWOTED_HOME`.
...
Run **exactly one** search, then read the JSON. Do not loop.

```bash
python3 qwoted_search.py --query "marketing automation" --max-hits 20
```

If `RESULT` is `rate_limited`, `too_soon_after_login`, or `not_warmed`,
tell the user. Do not pass `--http` to "just make it work."

Empty `--query` is refused unless `--dump-index` (do not use that on a
fresh account).
...
When the user replies "ok" / "yes" / "go", **do not burst-search**.
Move on to Stage 3 only for pages they approved, then Stage 4 as
dry-run drafts. If they said "go" during Stage 2, that means one
search, not twenty.
...
```bash
export QWOTED_HOME="$HOME/.qwoted-trial-1"   # throwaway isolation
...
python3 qwoted_pitch.py --source-request-id 235897 --pitch-text-file /tmp/p.txt --create-draft
