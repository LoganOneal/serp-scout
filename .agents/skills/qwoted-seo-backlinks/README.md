...
> **serp-scout local copy.** Read [`LEARNINGS.md`](LEARNINGS.md) and
> [`TRIAL.md`](TRIAL.md) before using this. Login is real Chrome
> (`--start-chrome` / `--browse`). Search defaults to in-page Algolia
> with caps and jitter. Pitch default is a local file, not a Qwoted
> API call. Do not loop `qwoted_search.py`. Do not `--http` or `--send`
> on an account you cannot lose.
